...
async function updateAccesssToken(refreshToken) {
  const { body } = await AutoScripts.HttpPost(
...
const SignIn = async () => {
  if (!AutoScript.RefreshTokenList?.length) throw '未提供任何账号'

  const TOKEN_CONFIG = 'data/third/aliyunpan-signin/config.json'

  const tokenConfig = (await AutoScripts.ignoredError(AutoScripts.Readfile, TOKEN_CONFIG)) || '{}'
  ...
  for (let i = 0; i < AutoScript.RefreshTokenList.length; i++) {
    const token = AutoScript.RefreshTokenList[i]
  ...
  await AutoScripts.Writefile(TOKEN_CONFIG, JSON.stringify(TokenMap, null, 2))
...
const onRun = async () => {
  ...
  AutoScripts.alert('签到信息', res.join('\n'))
}
